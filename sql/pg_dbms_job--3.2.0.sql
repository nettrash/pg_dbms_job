...
-- When a scheduled job is added or removed, wake the daemon to reread the table.
-- Toggle these triggers with dbms_job.set_notify(boolean); for a pure-polling,
-- NOTIFY-free deployment disable them and set the daemon's enable_notify=off.
...
-- reversible operation. With the triggers dropped, submits no longer emit
-- pg_notify -- eliminating pg_notify's cluster-wide commit-time serialization
-- and the risk that a lagging listener fills the shared async-notify queue and
-- fails submitting transactions. Pair this with the daemon's enable_notify=off
-- (which stops it from LISTENing) for a fully NOTIFY-free, poll-only deployment.
-- Idempotent: safe to call repeatedly and in either state.
CREATE PROCEDURE dbms_job.set_notify(enabled boolean)
    LANGUAGE PLPGSQL
    AS $$
BEGIN
    IF enabled THEN
...
----
-- Auto-schedule the run-details partition maintenance (new in 3.1.0)
--
-- maintain_run_details_partitions() creates next month's partition ahead of
-- time and prunes old ones. If it is never scheduled, every history row falls
-- into the DEFAULT partition, which then blocks creating the monthly partitions
-- and bloats without bound. Schedule it as a daily recurring job on install so
-- operators get correct behaviour by default. Idempotent: only submitted when
-- an equivalent recurring job does not already exist.
----
DO $$
BEGIN
    IF NOT EXISTS (
